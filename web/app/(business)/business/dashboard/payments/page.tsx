...
export default function BusinessPaymentHistoryPage() {
  const { user, isLoading: isAuthLoading } = useRequireRole(["USER"], { allowedAccountTypes: ["BUSINESS"], forbiddenRedirectTo: "/business/dashboard" });
  ...
  React.useEffect(() => {
    ...
    listPaymentsHistory()
      .then((data) => {
        setPayments(data.filter(p => p.customerId === user.id && p.bulkRequestId != null));
        setIsLoading(false);
      })
      .catch((err) => {
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="text-body text-neutral-500 mt-1">
            Review the payments you have received from recycling companies for your bulk waste.
          </p>
        </div>
        ...
        {status === "fail" && (
          <div className="bg-red-50 text-red-800 p-4 rounded-xl border border-red-200">
            Payment failed. Please contact the recycling company.
          </div>
        )}
        {status === "cancel" && (
