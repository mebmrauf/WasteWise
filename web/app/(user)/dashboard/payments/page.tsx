...
export default function CustomerPaymentHistoryPage() {
  const { user, isLoading: isAuthLoading } = useRequireRole(["USER"], { allowedAccountTypes: ["HOUSEHOLD"] });
  ...
  React.useEffect(() => {
    ...
    listPaymentsHistory()
      .then((data) => {
        setPayments(data.filter(p => p.customerId === user.id));
        setIsLoading(false);
      })
      .catch((err) => {
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="text-body text-neutral-500 mt-1">
            Review the payments you have received from collectors for your waste.
          </p>
        </div>
        ...
        {status === "fail" && (
          <div className="bg-red-50 text-red-800 p-4 rounded-xl border border-red-200">
            Payment failed. Please contact the collector.
          </div>
        )}
        {status === "cancel" && (
