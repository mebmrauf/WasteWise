...
import { rateBulkCollection } from "@/lib/api/marketplace";
import { z } from "zod";
...
export function CompanyRatingModal({ pickupId, companyName, onClose, onSuccess }: CompanyRatingModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await rateBulkCollection(pickupId, score, comment || undefined);
      ...
      >
        ...
        {submitState === "success" ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col p-6 sm:p-8">
            <h2 className="text-h2 text-neutral-900 mb-2">Rate Recycling Company</h2>
            <p className="text-body text-neutral-600 mb-8">
              How was your experience with {companyName ? <strong>{companyName}</strong> : "the recycling company"}?
            </p>
